...
import { LogoDots } from "@/components/LogoMark";
...
export default async function GraciasPage({
  ...
  const textoCompartir = `Ya mandé mi propuesta para ${categoria.nombre}, ¿y tú? · Zacatecas Piensa`;
  ...
  return (
    <div className="flex justify-center px-6 py-20 sm:px-10 lg:px-[60px] lg:py-28">
      <div className="flex w-full max-w-[720px] flex-col gap-6">
        <LogoDots size={22} gap={12} animated />
        ...
        <div className="inline-block w-fit rounded-sm border border-line-card bg-white px-4.5 py-3.5 font-mono text-[13px] tracking-[0.06em] text-ink-mute">
          FOLIO {folio}
        </div>
        <div className="mt-3 flex flex-wrap gap-3.5">
          ...
          <Link
            href="/"
            className="rounded-sm border-[1.5px] border-marino px-7 py-4 text-base font-medium text-marino hover:bg-marino hover:text-white"
          >
            Proponer en otro tema
          </Link>
        </div>
        <div className="mt-1.5 max-w-[34em] rounded-sm border-l-[3px] border-rojo bg-cream-soft px-4.5 py-3.5 text-sm leading-relaxed text-ink-mute">
          “{textoCompartir}”
        </div>
      </div>
    </div>
